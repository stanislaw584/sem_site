...
Workflow не создает Nginx server block для вашего домена, потому что домен, SSL и структура `/etc/nginx/sites-available` зависят от конкретного VPS.

Nginx-конфиг ниже нужно один раз добавить вручную на сервере. После этого CI будет только обновлять файлы сайта и делать `nginx reload`.
